...
function sniff ([String] $Options = "-n", [Int32] $Spacing = 0, [Switch] $AskWhichAdapter)
{
    ...
    { 
        ...
        #Special Case: ICMP 
        $icmp = $false #This must be here, filter is repeatedly called.
        ...
        #Split sender ip.port or hostname.port, but ICMP has no port.
        $leftsidedots = $leftside[-1] -split ".",0,"SimpleMatch"
        if ($leftsidedots.count -lt 2 -and $icmp -eq $false) { write-host $originalline -foreground $DefaultColor -background $GroundColor ; return } 
        
        #Split receiver ip.port or hostname.port, but ICMP has no port.
        $rightsidedots = $sides[1] -split ".",0,"SimpleMatch"  
        if ($rightsidedots.count -lt 2 -and $icmp -eq $false) { write-host $originalline -foreground $DefaultColor -background $GroundColor ; return } 
        ...
        #Print source IP and port, but ICMP uses no port numbers.
        if ($icmp)
        {
        ...
        #Print destination IP and port, but ICMP uses no port numbers.
        if ($icmp)
        {
    ...
    else
    {
        ...
        else 
        {
            ...
            $ifnum = @(windump.exe -D | select-string -NotMatch 'Virtual|Tunnel') 
            if ($ifnum.count -gt 0)
            {
